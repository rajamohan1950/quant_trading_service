            ...
            try:
                ...
                if result['pipeline_status'] == 'success':
                    ...
                    with col2:
                        ...
                        st.metric("Signal Strength", signal['signal_strength'])
                        st.metric("Execution Urgency", signal['execution_urgency'])
    ...
    if st.button("📥 Load Recent Ticks", key="load_ticks_btn"):
        try:
            ...
            query = """
                ...
                WHERE tick_generated_at >= datetime('now', '-1 hour')
...
def render_model_performance_tab(ml_pipeline):
    ...
    if st.button("📈 Evaluate Model Performance", key="eval_model_btn"):
        with st.spinner("🔄 Evaluating model performance..."):
            try:
                ...
                query = """
                    SELECT * FROM tick_data 
                    WHERE tick_generated_at >= datetime('now', '-24 hours')
                    ORDER BY tick_generated_at DESC 
                    LIMIT 1000
                """
                    ...
                    if not processed_data.empty and 'trading_label_encoded' in processed_data.columns:
                        ...
                        if 'error' not in evaluation_result:
                            ...
                            with col1:
                                ...
                                st.metric("Validation Samples", metrics['validation_samples'])
                            ...
                            with col2:
                                ...
                                for class_name in ['HOLD', 'BUY', 'SELL']:
                                    if class_name in metrics['precision']:
                                        ...
                                        st.write(f"  Precision: {metrics['precision'][class_name]:.3f}")
                                        st.write(f"  Recall: {metrics['recall'][class_name]:.3f}")
                                        st.write(f"  F1-Score: {metrics['f1_score'][class_name]:.3f}")
    ...
    if 'last_evaluation' in st.session_state:
        ...
        with col3:
            st.metric("Evaluation Time", evaluation['evaluation_timestamp'])
    ...
    if 'error' not in model_info:
        ...
        with col2:
            if model_info['active_model']:
                ...
                st.metric("Model Type", active_info['model_type'])
                st.metric("Features", active_info['feature_count'])
