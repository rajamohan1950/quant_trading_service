...
try:
    ...
    logging.warning(f"LightGBM not available ({e}), using demo model")
    ...
    def load_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if LIGHTGBM_AVAILABLE and os.path.exists(self.model_dir):
                ...
                logger.info(f"📁 Found {len(model_files)} LightGBM model files")
                ...
                for model_file in model_files:
                    try:
                        ...
                        if model_adapter.load_model():
                            ...
                            logger.error(f"❌ Failed to load LightGBM model: {model_name}")
                    
                    except Exception as e:
                        logger.error(f"❌ Error loading LightGBM model {model_file}: {e}")
                        continue
            
            # Set demo model as active by default
            if "demo_trading_model" in self.models:
                self.active_model = self.models["demo_trading_model"]
                logger.info(f"🎯 Active model set to: demo_trading_model")
                logger.info(f"📊 Active model info: {self.active_model.get_model_info()}")
            else:
                logger.error("❌ No demo model available to set as active")
